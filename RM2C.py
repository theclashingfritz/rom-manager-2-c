        ...
        if b and b[1]>b[0]:
            ...
            if not i:
                ...
                if b[0]>0x1220000:
                    ...
                    load = '\t' * tab + "LOAD_MIO0(0xA, " + name + "SegmentRomStart, " + name + "SegmentRomEnd),\n"
                else:
                    load = '\t' * tab + "LOAD_MIO0(0xA, " + banks[i] + "SegmentRomStart, " + banks[i] + "SegmentRomEnd),\n"
            else:
                load = '\t' * tab + "LOAD_MIO0(%d, " % banks[i][1] + banks[i][0] + "_mio0SegmentRomStart, " + banks[i][0] + "_mio0SegmentRomEnd),\n"
                load += '\t' * tab + "LOAD_RAW(%d, " % banks[i][2] + banks[i][0] + "_geoSegmentRomStart, " + banks[i][0] + "_geoSegmentRomEnd),\n"
        ...
        if model:
            ...
            if LevelSpecificModels.get(level) and Seg==0x12:
                for l in LevelSpecificModels[level].split("\n"):
                    ...
                    if model[1] == 'geo':
                        data += '\t' * tab + comment + "LOAD_MODEL_FROM_GEO(%d, %s),\n" % (i, lab)
                    else:
                        #Its just a guess but I think 4 will lead to the least issues
                        data += '\t' * tab + comment + "LOAD_MODEL_FROM_DL(%d, %s, 4),\n" % (i, lab)
...
def WriteArea(s, area, Anum, id):
    ...
    data += '\t' * tab + "TERRAIN_TYPE(%d),\n" % (area.terrain)
    data += '\t' * tab + "JUMP_LINK(local_objects_%s),\n" % (id)
    data += '\t' * tab + "JUMP_LINK(local_warps_%s),\n" % (id)
    if hasattr(area, 'macros'):
        data += '\t' * tab + "MACRO_OBJECTS('local_macro_objects_%s')," % id
    data += '\t' * tab + "END_AREA(),\n"
    data += '\t' * tab + "RETURN()\n"
    data += "};\n\n"
    ...
    for o in area.objects:
        ...
        if o:
            if o[4] == 255 and 'Scroll_Texture' in o[-2]:
                ...
                comment=''
            data += '\t' * tab + comment + "OBJECT_WITH_ACTS({}, {}, {}, {}, {}, {}, {}, {}, {}, {}),\n".format(*o)
...
def ExportObjects(reg,Objects,rom,ass,rootdir,editor):
    ...
    if type(reg)==list:
        for bhv,o in Objects.items():
            ...
            if any(r):
                [col,funcs] = ExportBhv(rom,o,bdata,bhv,0,f,editor)
                if col:
        ...
        for bhv,o in Objects.items():
            [col,funcs,new] = ExportBhv(rom,o,bdata,bhv,0,f,editor)
            if col:
                collisions.append([col,o,bhv,new])
            if funcs:
                functions.extend(funcs)
        ...
        for bhv,o in Objects.items():
            [col,funcs,new] = ExportBhv(rom,o,bdata,bhv,1,f,editor)
            if col:
                collisions.append([col,o,bhv,new])
            if funcs:
                functions.extend(funcs)
...
def ExportActorCol(col,reg,rom,ass,C):
...
def ExportBhv(rom,o,bdata,bhv,check,f,editor):
    Bhvs=[[o[1],o[-1],bhv]]
...
""")
    for k, v in Trajectories.items():
        ...
        if k == 'ccm_seg7_trajectory_penguin_race_RM2C' or k == 'ccm_seg7_trajectory_snowman_RM2C':
            ...
            Dat = (AddrBase | UPA(rom, v[1], '>H', 2)[0])
        elif k == 'bob_seg7_metal_ball_path0_RM2C' or k == 'bob_seg7_metal_ball_path1_RM2C' or k == 'ttm_seg7_trajectory_RM2C':
            AddrBase = UPA(rom, v[0] + 2, '>H', 2)[0] << 16
            Dat = (AddrBase | UPA(rom, v[1] + 2, '>H', 2)[0])
        elif k == 'sThiHugeMetalBallTraj_RM2C' or k == 'sThiTinyMetalBallTraj_RM2C':
            AddrBase = UPA(rom, v + 2, '>H', 2)[0] << 16
            Dat = (AddrBase | UPA(rom, v + 6, '>H', 2)[0])
            DirectLoad = True
        else:
            Dat = UPA(rom, v, '>L', 4)[0]
        ...
        try:
            if Dat >> 24 == 0x7:
                NewTraj += '\n//%s Has a level embedded value, Default fallback used.\n' % k
                ...
                NewTraj += 'const Trajectory {}_path[] = {{\n'.format(k)
                x = 0
                while(True):
                    point = UPA(rom, Dat + x, '>4h', 8)
                    if point[0] <= -1:
                        break
                    # If tihs passes, The trajectory is corrupted!
                    if point[0] != 0 and x == 0:
                        NewTraj += '\t// This trajectory was either read wrong or corrupted! No postional output was written as a result.\n'
                        break
                    NewTraj += '\tTRAJECTORY_POS({}, /*pos*/ {}, {}, {}),\n'.format(*point)
                    x += 8
                NewTraj += '\tTRAJECTORY_END(),\n};\n'
        except:
...
def AsciiConvert(num):
    if num < 10: #numbers start at 0x30
        return chr(num+0x30)
    elif num < 0x24: #capital letters start at 0x41
        return chr(num+0x37)
    elif num < 0x3E: #lowercase letters start at 0x61
        return chr(num+0x3D)
    return TextMap[num]
